...
# create Figure 5: 3 heatmaps for m & a at L=0, L=0.5, L=1------------------------
...
drawHeatMap.am.model2.rec <- function(m.max, t.max, n, a.max, r, b, c, u, v, L, grid.size, showplot = TRUE, showlegend = TRUE) {
  ...
  heat.plot <- ggplot(data = heat, aes(x = a.heat, y = m.heat)) + geom_tile(aes(fill = value.heat)) + labs(x = "a, acculturation", y = "m, migration") + ggtitle(paste("n = ", n, ", L = ", L, ", r = ", r, ", b = ", b, ", c = ", c, ", u = ", u, ", v = ", v, sep = "")) + theme_grey(base_size = 18) + theme(plot.title = element_text(size = 16, hjust = 0.5), legend.text = element_text(size = 10), legend.position = leg, axis.text = element_text(size = 12), axis.line = element_blank()) + scale_fill_viridis(name="p") + scale_x_continuous(breaks = seq(0,1,by=0.2), expand = c(0,0)) + scale_y_continuous(expand = c(0,0))
...
heat.am <- drawHeatMap.am.model2.rec(m.max = 0.6, t.max = 1000, n = 5, a.max = 1, r = 0.0, b = 1, c = 0.2, u = 0.1, v = 0.5, L = 1, grid.size = 3)

# create 3 heat plots in Fig 5 for different L
...
heat2.rec <- drawHeatMap.am.model2.rec(n = n.heats, r = r.heats, t.max = t.max.heats, m.max = m.max.heats, a.max = a.max.heats, b = b.heats,  c = c.heats, u = u.heats, v = v.heats, L = 0.5, grid.size = grid.size.heats, showplot = F, showlegend = F)[[2]]
heat3.rec <- drawHeatMap.am.model2.rec(n = n.heats, r = r.heats, t.max = t.max.heats, m.max = m.max.heats, a.max = a.max.heats, b = b.heats,  c = c.heats, u = u.heats, v = v.heats, L = 1, grid.size = grid.size.heats, showplot = F, showlegend = T)[[2]]

# use cowplot to create composite plot
legend <- get_legend(heat3.rec)
heat3.rec <- heat3.rec + theme(legend.position="none")
heat1.rec <- heat1.rec + ggtitle("L = 0")
heat2.rec <- heat2.rec + ggtitle("L = 0.5")
heat3.rec <- heat3.rec + ggtitle("L = 1")
...
# save to file
save_plot("model2_rec_am_heats_varyL.png", all.heats.rec, base_width = 30, base_height = 10, units = "cm")


# create Figure 6: 3 heatmaps for m & L at v=0.4, v=0.5, v=0.6------------------------
...
drawHeatMap.Lm.model2.rec <- function(m.max, t.max, n, a, r, b, c, u, v, L.max, grid.size, showplot = TRUE, showlegend = TRUE) {
  ...
  heat.plot <- ggplot(data = heat, aes(x = L.heat, y = m.heat)) + geom_tile(aes(fill = value.heat)) + labs(x = "L, payoff-biased copying", y = "m, migration") + ggtitle(paste("a = ", a, ", n = ", n, ", r = ", r, ", b = ", b, ", c = ", c, ", u = ", u, ", v = ", v, sep = "")) + theme_grey(base_size = 18) + theme(plot.title = element_text(size = 16, hjust = 0.5), legend.text = element_text(size = 10), legend.position = leg, axis.text = element_text(size = 12), axis.line = element_blank()) + scale_fill_viridis(name="p") + scale_x_continuous(breaks = seq(0,1,by=0.2), expand = c(0,0)) + scale_y_continuous(expand = c(0,0))
...
# create 3 heat plots in Fig 6 for different v
